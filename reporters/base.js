 ...
 *     Error property
 */
exports.list = function (runner, failures) {
    ...
    failures.forEach(function (test, i) {
        ...
        BaseConsoleLog(fmt, i + 1, testTitle, msg, stack);
        BaseConsoleLog();
    });
};
 ...
 * @memberof Mocha.reporters
 */
Base.prototype.epilogue = function (runner) {
    var stats = this.stats;
    var fmt;

    BaseConsoleLog = runner.LOG;
    ...
    BaseConsoleLog(fmt, stats.passes || 0, milliseconds(stats.duration));

    // pending
    if (stats.pending) {
        fmt = color('pending', ' ') + color('pending', ' %d pending');

        BaseConsoleLog(fmt, stats.pending);
    }
    ...
    if (stats.failures) {
        ...
        BaseConsoleLog(fmt, stats.failures);

        Base.list(runner, this.failures);
        BaseConsoleLog();
    }
